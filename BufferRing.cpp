...
void  CBufferRing::putBufferRing(Buffer* buf, int input, int kind)
{
    ...
    if (kindData[pos] == TEXT_HALFWAY && isText_Buffer(dim[0]) && inputSrc[pos] == input 
                                      && input != OPERATION_DATA && kind == UNKNOWN_DATA) {
    ...
    for (i = nxt; i < dim->state; i++) {
        ...
        if      (input == INFO_DATA)  cat_s2Buffer("INFO: ", &pBuf[pos]);
        else if (input == WARN_DATA)  cat_s2Buffer("WARN: ", &pBuf[pos]);
        else if (input == ERROR_DATA) cat_s2Buffer("ERR : ", &pBuf[pos]);
